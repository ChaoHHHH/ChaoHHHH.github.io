...
# 读取全部结构
all_data = read("./mace.xyz", index=":")
print(f"共有{len(all_data)}个结构")

# 划分数据集
def split_all_data(strus, trp, vp, tsp):
    data = strus.copy()
    np.random.seed(42)
    ...
    tr = int(n * trp)
    v  = int(n * vp)
    return data[:tr], data[tr:tr+v], data[tr+v:]

trd, vd, ted = split_all_data(all_data, train_proportion, validation_proportion, test_proportion)

write("train.xyz", trd)
write("validation.xyz", vd)
write("test.xyz", ted)
