## 数据集
任何涉及机器学习的模型，第一步就是获取足够优质的数据集用于训练与测试。比如可以使用一个Host（符合条件的结构即可，比如说我的目的是Amor-SiO2-Si，那么我使用一个该结构就可以）结构，对于此结构做一些扰动，然后分别对这些扰动跑一小段小的分子动力学来获取大量结构；或者，直接对该Host结构使用MACE训练好的一些通用势跑大量的分子动力学；亦或者单纯的对Host结构做大量的扰动，这些方法都可以作为获取大量初始结构的办法。但是不建议跑长时间的AIMD，极其消耗资源，而且获得的结构丰富度也有限。

此处使用到的结构主要如下，还包括一些Si的结构，Amor-SiO2的结构这些都不是必要的，只是我顺便算了，本着不浪费的原则，一并使用了。

![1](../figs/20260707/image-1.png)

![2](../figs/20260707/image-2.png)

在获取到这些结构后，需要使用DFT软件（VASP、ABACUS、PWMAT、...）对其进行一次SCF计算，**输出原子的受力以及结构的应力信息**。

完成上述计算后，需要对这些数据进行处理，我们可以自己写代码处理，也可以使用模型自带的一些脚本，不同MLIP软件对数据集的格式要求可能不一样，需要看情况处理，但一般而言，可以统一转换成`.(ext)xyz`格式的文件，我用到的MLIP几乎都支持这个格式，即便遇到不支持的，处理起来也方便。

## 训练
在拿到上述数据后，比如`data.xyz`文件，就可以开始执行MACE的训练了（确保你安装了MACE），在文章末尾我给出了一个可用的`train.py`代码，供参考。或者也可以直接命令行参数提交，可以查看MACE官方的教程。

**建议查看[2026-07-08-MACE训练_2.md](./2026-07-08-MACE训练_2.md)使用其中的方法进行训练**

## 结果

这是最主要的输出部分，确保loss在稳定减少
```
...
2026-07-05 09:54:04.960 INFO: Epoch 187: head: Default, loss=0.00727528, RMSE_E_per_atom=    0.50 meV, RMSE_F=   27.23 meV / A
2026-07-05 09:57:50.697 INFO: Epoch 188: head: Default, loss=0.00728713, RMSE_E_per_atom=    0.51 meV, RMSE_F=   27.25 meV / A
2026-07-05 10:01:37.270 INFO: Epoch 189: head: Default, loss=0.00725387, RMSE_E_per_atom=    0.51 meV, RMSE_F=   27.18 meV / A
2026-07-05 10:05:24.289 INFO: Epoch 190: head: Default, loss=0.00726462, RMSE_E_per_atom=    0.50 meV, RMSE_F=   27.21 meV / A
2026-07-05 10:09:10.491 INFO: Epoch 191: head: Default, loss=0.00727438, RMSE_E_per_atom=    0.51 meV, RMSE_F=   27.23 meV / A
2026-07-05 10:12:57.668 INFO: Epoch 192: head: Default, loss=0.00725913, RMSE_E_per_atom=    0.60 meV, RMSE_F=   27.18 meV / A
2026-07-05 10:16:43.737 INFO: Epoch 193: head: Default, loss=0.00732247, RMSE_E_per_atom=    0.97 meV, RMSE_F=   27.20 meV / A
...
```

模型在训练集和验证集上的最终表现，同时也会有测试集的信息，但是太长了此处不给出
```
2026-07-05 10:41:05.005 INFO: Error-table on TRAIN and VALID:
+---------------+---------------------+------------------+-------------------+
|  config_type  | RMSE E / meV / atom | RMSE F / meV / A | relative F RMSE % |
+---------------+---------------------+------------------+-------------------+
| train_Default |            3.7      |         23.5     |          2.06     |
| valid_Default |            3.7      |         28.2     |          2.47     |
+---------------+---------------------+------------------+-------------------+
```
训练过程中的Loss曲线
![3](../figs/20260707/mace_model_run-42_train_Default_stage_two.png)


## 流程
总之，整个流程比较困难的地方只在第一步，我们需要使用合适的方法获取到足够丰富的数据集，然后对这些结构批量进行SCF计算，最后获取受力信息。这些工作一旦完成，剩下的就是交给MLIP模型自己训练即可。当然为了获取更好的模型，还可以对模型的参数进行调整，但一般而言，默认参数基本是足够的。
...
CONFIG = {
    ...
    # 方式二: 从单个文件自动拆分 (推荐)
    "split_source": "mace.xyz",                       # 原始 .xyz 文件, 设为 None 则使用上面的 train_file
    ...
    "batch_size": 2,              # 批大小
    "max_num_epochs": 200,         # 最大训练轮数
    "seed": 42,                   # 随机种子
    "start_swa": 100,               # SWA 阶段起始 epoch
    "patience": 2048,              # 早停耐心
    "distributed":True,
